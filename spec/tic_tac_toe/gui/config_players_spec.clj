...
(describe "configure players"
  ...
  (context "config x-type"
    (it "Invokes update state with option 1 if button 1 is clicked"
      (let [event {:x 144 :y 350}
            state {:status :config-x-type :interface :gui}]
  ...
  (context "config o-type"
    (it "Invokes update state with option 1 if button 1 is clicked"
      (let [event {:x 144 :y 350}
            state {:status :config-o-type :interface :gui}]
        (core/get-selection state event)
        (should-have-invoked :update-state {:with [state 1]})))
  ...
  (context "config x-difficulty"
    (it "Invokes update state with option 1 if button 1 is clicked"
      (let [event {:x 288 :y 288}
            state {:status :config-x-difficulty :interface :gui}]
        (core/get-selection state event)
        (should-have-invoked :update-state {:with [state 1]})))
    ...
    (it "Returns nil if no valid button is clicked"
      (let [event {:x 1 :y 1}]
        (should-be-nil (core/get-selection {:status :config-x-difficulty :interface :gui} event))))
    )
  ...
  (context "config o-difficulty"
    ...
    (it "Returns nil if no valid button is clicked"
      (let [event {:x 1 :y 1}]
        (should-be-nil (core/get-selection {:status :config-o-difficulty} event))))
    )
  #_(it "moves to config o type and sets x-type to Human if human button clicked on config-x-type"
      (let [event     {:x 144 :y 350}
            new-state (core/get-selection (test-core/state-create {:status :config-x-type}) event)]
  ...
  #_(it "moves to config o type if x-difficulty is set to easy"
      (let [event     {:x 288 :y 288}
            new-state (core/get-selection (test-core/state-create {:status :config-x-difficulty :x-type :computer}) event)]
  ...
  #_(context "config o"
      (it "moves to select-board if o-type is set to human"
        (let [event     {:x 144 :y 350}
              new-state (core/get-selection (test-core/state-create {:status :config-o-type}) event)]
      ...
      (it "moves to select-board after o-difficulty is set to easy"
        (let [event     {:x 288 :y 288}
              new-state (core/get-selection (test-core/state-create {:status :config-o-difficulty :o-type :computer}) event)]
